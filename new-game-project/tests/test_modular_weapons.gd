...
func test_weapon_base_interface_compliance() -> void:
	var plasma = PLASMA_SCENE.instantiate()
	assert_true(plasma is WeaponBase, "OrbitingPlasma inherits WeaponBase")
	assert_eq(plasma.weapon_id, "weapon_plasma", "Plasma weapon_id is set")
	var p_stats: Dictionary = plasma.get_stats()
	assert_true(p_stats.has("effective_damage"), "Plasma stats has effective_damage")
	assert_true(p_stats.has("effective_cooldown"), "Plasma stats has effective_cooldown")
	assert_true(plasma.has_method("tick"), "Plasma implements tick")
	assert_true(plasma.has_method("fire"), "Plasma implements fire")
	plasma.free()

	var thunder = THUNDER_SCENE.instantiate()
	assert_true(thunder is WeaponBase, "ThunderStrike inherits WeaponBase")
	assert_eq(thunder.weapon_id, "weapon_thunder", "Thunder weapon_id is set")
	var t_stats: Dictionary = thunder.get_stats()
	assert_true(t_stats.has("effective_damage"), "Thunder stats has effective_damage")
	assert_true(t_stats.has("effective_cooldown"), "Thunder stats has effective_cooldown")
	assert_true(thunder.has_method("tick"), "Thunder implements tick")
	assert_true(thunder.has_method("fire"), "Thunder implements fire")
	thunder.free()
